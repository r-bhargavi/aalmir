...
class resCompany(models.Model):
    ...
    def price_update_products(self):
        wo=self.env['mrp.production.workcenter.line'].search([])
        for each_wo in wo:
            if each_wo.raw_materials_id:
                for rm in each_wo.raw_materials_id:
                    if rm.percent_rm>0.0:
                        rm.write({'percent_value':rm.order_id.each_batch_qty*rm.percent_rm/100,'percent_value_25':rm.order_id.each_batch_qty*rm.percent_rm/2500})
        return True
...
#        cust_products=self.env['customer.product'].search([])
#        for each in cust_products:
#            each.product_tmpl_id=each.product_id.product_tmpl_id.id
#        return True
...
##        all_prods = self.env['product.template'].search([('id','=',3676)])
#        count=1
#
#        for each in all_prods:
#            product_id=self.env['product.product'].search([('product_tmpl_id','=',each.id)],limit=1)
##            product_id=self.env['product.product'].search([('id','=',3676)],limit=1)
#            if product_id:
...
#                            WHERE id IN %s and fixed_price>0.0 group by currency_id###""", (tuple(pricelist_id.ids),))
