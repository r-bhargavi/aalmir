...
#				('purchase', 'Purchase'),
			...
			help="'New': Production request created but not sent to production\n\
                            ...
                            'Scheduled': MO created, scheduled and confirmed for raw material request\n\
...
#            point1 in bom changes
    ...
    def send_for_approval(self):
        ...
        if temp_id:
                ...
                send_user = ",".join(recipient_partners)
                new_subject='API-MRP BOM Approval Alert: BoM Approval Requested for %s.'%str('['+self.product_id.default_code+']'+' '+self.product_id.name)
                body_html = """<div> 
                        <p>Dear User,<br/>
...
#                self.message_post(body=body_html)
    ...
    def send_approval_reminder(self):
        ...
        if temp_id:
                ...
                send_user = ",".join(recipient_partners)
                new_subject='API-MRP BOM Approval Reminder Alert: BoM Approval Reminder for %s.'%str('['+self.product_id.default_code+']'+' '+self.product_id.name)
                body_html = """<div> 
                        <p>Dear User,<br/>
                        <p>Kindly Approve BOM at your earliest so that MO can be planned and started.</p><br/>
		    			<p>Product:<b>%s</b> </p>
